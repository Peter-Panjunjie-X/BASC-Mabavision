...
# 新加入的模块
from modules.BARM_module import BARM
from modules.SCEM_module import SCEM
...
@MODELS_MMSEG.register_module()
@MODELS_MMDET.register_module()
class MM_mamba_vision_BASC(MambaVision):
    def __init__(self, 
                 dim,
                 in_dim,
                 depths,
        ...
        # 新加入的模块
        # ===== BARM =====
        self.barm = BARM(self.dims[0])
        # ===== SCEM =====
        self.scem = SCEM(self.dims[3])
    ...
    # 带permute的forward函数，方便在特定阶段加入BARM和SCEM模块
    # def forward(self, x):
    #     x = self.patch_embed(x)
    #     outs = []
    #     for i, level in enumerate(self.levels):
